...
- **CONFIG projet 107 — câblage org Salesforce** : 107 avait été créé sans org (106, premier essai, abandonné). Renseigné `sf_username=<org-salesforce>`, `sf_instance_url=…orgfarm-2b37d18797-dev-ed…`, `sf_org_id=00Dfj00000VIwy9EAD`. Vérifié : `create_sf_admin_service(107)` OK, `jordan.sf_admin_service`/`sfdx_service` OK, org `connectedStatus: Connected ». Débloque « SF Admin service not initialized ». ⚠️ `git_service` reste None (`git_repo_url`/`GIT_TOKEN` à câbler pour phases 2-6).
