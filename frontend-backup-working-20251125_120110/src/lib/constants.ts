...
/**
 * Organization types for Salesforce implementation
 */
...
/**
 * Get avatar path for agent (small = 120x120, large = 400x400)
 */
export function getAgentAvatar(agentId: string, size: 'small' | 'large' = 'small'): string {
  const agent = AGENTS.find(a => a.id === agentId);
  if (!agent) return '/avatars/small/olivia-ba.png'; // fallback
  
  const filename = agent.avatar.split('/').pop();
  return `/avatars/${size}/${filename}`;
}
