...
| GL-23 | **Bloquant** | Écart de prix Pro : Stripe facture **49 €/mois** (`price_1TRW5X2U0jLqzz5TWvNKYxow`, sandbox) alors que le site annonce **79 € HT/mois**. Mesuré le 17/09 pendant la souscription de bout en bout — la page de paiement affiche « Subscribe to Pro €49.00 per month ». **Décision Sam 17/09 : on reste à 79 €.** Fait en sandbox le 17/09 : prix `price_1UGhEN2U0jLqzz5TGaGQ7m58` créé à 79 €/mois, posé en prix par défaut du produit Pro, ancien prix 49 € désactivé, `STRIPE_PRICE_ID_PRO` mis à jour (sauvegarde `env.pre-prix-pro-79-20260917`), backend redémarré ; page de paiement vérifiée : « Subscribe to Pro €79.00 per month ». Team déjà à 1 490 €, correct. **Reste : refaire la même opération sur le compte de production — qui n'existe pas encore (voir GL-27).** | 🟡 |
...
| GL-27 | **Bloquant (si Pro ouvert)** | Le compte Stripe de **production n'est pas configuré** : aucun produit, aucun prix, guide d'installation encore à « choisissez votre modèle économique » (capture Sam, 17/09). L'application est branchée sur des clés `sk_test_`/`pk_test_` : aujourd'hui rien ne peut encaisser un paiement réel. Historique vérifié : les produits Pro et Team et le prix à 49 € datent du 29/04 (Sam) ; Claude n'a créé que le prix à 79 € le 17/09, en sandbox. | Chemin complet, à faire dans cet ordre quand Pro sera ouvert : (1) activer le compte Stripe (identité, IBAN, modèle économique — délai de vérification Stripe de quelques heures à quelques jours) ; (2) créer les produits Pro 79 € et Team 1 490 € côté live ; (3) nouvelles clés `sk_live_`/`pk_live_` dans `.env` ; (4) nouveau point de terminaison webhook + nouveau `STRIPE_WEBHOOK_SECRET` ; (5) rejouer une souscription complète en live (carte réelle, remboursée) ; (6) vérifier que la page de paiement affiche 79 €. **Hors chemin critique si l'ouverture du 1er est Free seul.** | 👤 puis ❌ |
...
|---|---|---|---|
| **A — Free seul le 1er, Pro quand le solde le permet** | Free (Nemotron, coût marginal nul) + liste d'attente Pro | Rien de plus ; le site annonce Pro « bientôt » comme aujourd'hui pour Team. **Sort GL-25 (crédits Anthropic), GL-23 et GL-27 (Stripe live) du chemin critique du 1er octobre.** | Aucun risque client ; retarde le premier euro |
| **B — Pro ouvert, solde rechargé avant le 1er** | Free + Pro | Une recharge Anthropic suffisante pour absorber les premiers clients (2 SDS/mois inclus par client) **et** l'activation complète du compte Stripe live (GL-27, délai de vérification Stripe non maîtrisé) | Si le solde tombe pendant le mois, panne visible chez un client payant ; l'activation Stripe peut ne pas être prête à temps |
