...
> **▶ PROCHAINE ACTION (10 juin)** : (1) Sam crée les 3 projets R&D (Pipeline Tuner / Grid Foresight / Omnichannel Loop) via wizard studio. (2) Merger `fix/exec159-assembly` (FIX-PARSE-001 + FIX-MAXTOKENS-002 + FIX-EMPTY-001 ✅ prouvés) puis redémarrer backend+worker. (3) Batch Vague 2. Restent ouverts (non bloquants batch) : PATCH-MERGE-001, FEAT-LANG-001, OBSERV-001.
...
|----|------|-------------|----------------------|--------|
| FIX-PARSE-001 | 🔴 P1 | `_parse_raw_markdown_json` renvoie `unrecoverable` sur un JSON tronqué au lieu de fermer les structures ouvertes → sections expert vides alors que le contenu existe en base. **Le plus rentable + rétroactif.** | `tools/lib/collect_sds.py` (fonction `_parse_raw_markdown_json`, étape « 5 » de récup défaillante). **PROUVÉ** : fermeture des structures récupère Aisha 98,9% (54701/55322) et Elena 98,5% (49493/50240). Fix → **re-render SDS exec 159 remplit Aisha+Elena sans réexécution.** Smoke test = vérifier les 2 sections présentes. | ✅ FAIT (10 juin, branche `fix/exec159-assembly`, commit `666e02d`) — prouvé : Aisha 100% / Elena 99,9% récupérés, SDS 159 re-rendu 428K (+51K) |
| FIX-MAXTOKENS-002 | 🔴 P1 | Specs expert tronquées en fin de JSON car `max_tokens` trop bas / pas de streaming sur ces chemins. Empêche la récidive. | Aisha `salesforce_data_migration.py:341` (=16000), Elena `salesforce_qa_tester.py:642` (=16000). **+ chemin patch archi** : `salesforce_solution_architect.py:574` — ajouter `'patch'` à `max_out = 64000 if mode in (...)` (sinon `else 16000` → troncature data_model patch). Autres `max_tokens=16000` repérés : apex 236/251/454/469, trainer 270, devops 236, admin 263/362, lwc 397/412/577, pm 371. | ✅ FAIT (10 juin, `397ed4a`) — Aisha/Elena spec 64K + mode patch Marcus 64K. Autres 16000 repérés laissés tels quels (hors périmètre minimal). |
| PATCH-MERGE-001 | 🟠 P2 | Le patch `automation_design` ne renvoie que `flows` (20, ex-30) et **supprime** `apex_triggers`/`scheduled_jobs`/`platform_events` (le merge remplace au lieu de fusionner). | Template `marcus_architect/patch` (`get_patch_prompt`, `salesforce_solution_architect.py:450`) + logique de merge. Imposer le renvoi de la section COMPLÈTE et/ou deep-merge. | ✅ FAIT (10 juin, branche fix/patch-merge-001) — merge preservant les cles (2 sites REVISION-001) + YAML patch 64K, teste unitairement |
| FIX-EMPTY-001 | 🟠 P2 | Jordan/DevOps : appel LLM (interaction 1368) renvoie **chaîne vide, 0 token, `success=true`** → deliverable `{raw_markdown: ""}` stocké en silence. | `salesforce_devops.py`. Retry sur réponse vide + échouer bruyamment au lieu de stocker du vide. | ✅ FAIT (10 juin, `31ba2d5`) — retry x2 + RuntimeError, testé unitairement (mock : retry récupère / vide persistant lève). |
| FEAT-LANG-001 | 🟠 P2/produit | Brief FR → toutes les specs en EN. Aucune directive de langue transmise aux agents ; template SDS figé `lang="en"`. | Injecter la langue du projet dans les system prompts agents + rendre `lang` dynamique dans `sds_shell.html.j2`. (Elena avait glissé un bout de FR « Ma facture du mois dernier est incorrecte » → le brief FR transparaît mais l'agent rédige EN par défaut.) | ✅ FAIT (10 juin, branche feat/lang-001) — directive langue dans llm_service (sync+async, cache lru), colonne language ajoutee au modele Project (existait en DB seulement), lang dynamique sds_shell. Teste : exec 159 -> fr, SDS rendu <html lang=fr> |
| OBSERV-001 | 🟡 P3 | Appels Sonnet très longs sans aucun log (WBS = **806 s / 173K tokens**, gap 457s) → indiscernable d'un worker planté (on a failli redémarrer un run sain). | Ajouter un heartbeat log (1 ligne / 30-60s) pendant les appels streaming. + **réactiver le monitoring N8N** (rien n'a détecté la mort du worker la semaine passée). | ✅ FAIT (10 juin, branche feat/observ-001 + infra) — heartbeat 30s dans le streaming (teste, PING_OK). N8N : monitoring reactive (cause reelle de la desactivation = noeud executeCommand retire par defaut en n8n 2.x ; fix NODES_EXCLUDE=[] dans override.conf) + digital-humans-worker AJOUTE au perimetre du check (il manquait — trou qui a masque la mort du worker) |
