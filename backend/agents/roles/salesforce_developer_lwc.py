...
"""
    ...
    if LLM_SERVICE_AVAILABLE:
        print(f"🤖 Calling Claude API (Haiku - lwc tier)...", file=_sys.stderr)
        _llm_resp = generate_llm_response(
            prompt=full_prompt,
            agent_type="lwc",
            system_prompt=LWC_PROMPT[:4000],  # Truncate if too long
            max_tokens=16000,
            temperature=0.3
        )
...
if __name__ == "__main__":
    ...
    if args.use_rag and RAG_AVAILABLE:
        try:
            ...
            rag_context = get_salesforce_context(query, n_results=5, agent_type="lwc_developer")
