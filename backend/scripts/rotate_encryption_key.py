...
# Prefixes of provider tokens that must never sit in the clear in
# `encrypted_value` (cla:SEC-03 — jordan_deploy_service still tolerates them).
...
def _looks_like_plaintext_token(value: str) -> bool:
    return value.startswith(PLAINTEXT_TOKEN_PREFIXES)
...
def classify(
    ...
    if new is not None:
        try:
            ...
            pass
    if allow_plaintext and _looks_like_plaintext_token(value):
...
def main() -> int:
    ...
    if not args.verify_only and not args.new_key:
        parser.error("--new-key is required unless --verify-only is used.")
    ...
    new_fernet = build_fernet(args.new_key) if args.new_key else None
    ...
    print(f"New key       : {'provided' if new_fernet else '(none — verify only)'}")
    print("=" * 72)
